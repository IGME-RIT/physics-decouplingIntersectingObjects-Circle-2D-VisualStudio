/*
Title: Detecting the Minimum Translation Vector (Circle - 2D)
File Name: main.cpp
Copyright © 2015
...
Description:
This is a demonstration of decoupling intersecting circles using the minimum translation vector 
in 2D. There are Pink and Yellow wireframes of circles, when the circles overlap, the intersection
will be separated making sure that the object which moved maintains it's current position. This can be
easily reversed to simulate walking into a wall rather than pushing an object.

After the separating axis theorem is detected and the overlap is measured it is very simple to decouple objects.
Normally, you would want to take a ratio of the magnitude of each objects velocity in the direction of the minimum
translation vector to the sum of the magnitude of the velocities in that direction. The resulting ratio scaled
by the overlap is the magnitude each object must translate to realistically decouple the collision.
Note: This simulation does not have velocities, so the object which is not being moved contributes to 100%
of the decoupling.
...
struct Circle
{
	...
	Circle(const glm::vec3& c, float r)
	{
		center = c;
		radius = r;
	}
